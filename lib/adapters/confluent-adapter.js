...
class ConfluentAdapter extends KafkaAdapter {
  ...
  _buildConfig() {
    ...
    if (this.config.sasl) {
      ...
      if (mech === 'OAUTHBEARER') {
        this._applyOAuth(root, this.config.sasl.oauth);
      } else {
  ...
  /**
   * Map declarative OAuth config onto librdkafka's native OIDC properties.
   *
   * librdkafka's built-in OAUTHBEARER OIDC support (sasl.oauthbearer.method=oidc)
   * implements the Client Credentials grant only. The Resource Owner Password
   * grant has no native equivalent, so it is rejected with guidance to use the
   * kafkajs backend (which fetches the token in-process and supports it).
   */
  _applyOAuth(root, oauth) {
    if (!oauth) {
      throw new Error(
        'OAUTHBEARER selected but no OAuth token endpoint configured. ' +
        'Set the token URL, grant type and client credentials on the broker.'
      );
    }
    const grantType = oauth.grantType || 'client_credentials';
    if (grantType !== 'client_credentials') {
      throw new Error(
        `OAUTHBEARER grant '${grantType}' is not supported on the confluent backend ` +
        `(librdkafka native OIDC only supports client_credentials). ` +
        `Switch the broker Backend to 'kafkajs' to use the '${grantType}' grant.`
      );
    }
    if (!oauth.tokenEndpoint) throw new Error('OAUTHBEARER: token endpoint URL is required');
    if (!oauth.clientId) throw new Error('OAUTHBEARER: client ID is required');

    root['sasl.oauthbearer.method'] = 'oidc';
    root['sasl.oauthbearer.token.endpoint.url'] = oauth.tokenEndpoint;
    root['sasl.oauthbearer.client.id'] = oauth.clientId;
    if (oauth.clientSecret) root['sasl.oauthbearer.client.secret'] = oauth.clientSecret;
    if (oauth.scope) root['sasl.oauthbearer.scope'] = oauth.scope;
    // librdkafka has no native 'audience' OIDC property; audience is only
    // honored on the kafkajs backend. Omitted here rather than silently
    // mis-mapped.
  }
}
