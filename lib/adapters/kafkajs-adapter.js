...
const codecs = require('../kafkajs-codecs');
const { createOAuthBearerProvider } = require('../oauth/oauth-token-provider');
...
function resolveCompression(compression) {
  if (compression == null || compression === 'none') return CompressionTypes.None;
  if (!(compression in COMPRESSION_MAP)) {
    throw new Error(`Unknown compression type: ${compression}`);
  }
  if (!codecs.isAvailable(compression)) {
    const pkg = codecs.packageFor(compression);
    throw new Error(
      `${compression.toUpperCase()} compression codec is not registered. ` +
      `Install the optional dependency '${pkg}' (e.g. 'npm install ${pkg}') ` +
      `in your Node-RED user directory and restart Node-RED. ` +
      `Note: this codec is also required on the consumer side to decode ` +
      `messages compressed with ${compression.toUpperCase()} by other producers.`
    );
  }
  ...
  _buildSASLConfig() {
    ...
    switch (this.config.sasl.mechanism) {
      ...
      case 'scram-sha-512':
        ...
        break;
      case 'oauthbearer':
        // Accept a pre-built provider function (advanced/programmatic use), or
        // build one from the declarative OAuth config supplied by the node.
        if (typeof this.config.sasl.oauthBearerProvider === 'function') {
          sasl.oauthBearerProvider = this.config.sasl.oauthBearerProvider;
        } else if (this.config.sasl.oauth) {
          sasl.oauthBearerProvider = createOAuthBearerProvider(this.config.sasl.oauth);
        } else {
          throw new Error(
